...
@@E-mail ID(s) TO Notify={alert_recipients}@@
@@Site={site}@@
@@Created Date={date}@@
...
with open("tasking.csv") as file:
    ...
    next(reader)  # Skip header row
    count = 0
    for subject, issue, details, request_type, ticket_status, reporting_mode, ticket_assignment_level, service_group, assigned_technician, service_category, alert_recipients, site, date, scheduled_end_time, response_due_by_time, impact, impact_details, urgency, category, subcategory, item, scheduled_start_time, dueby_date, request_closure_code, request_closure_comment, in reader:
        count+= 1
